...
// Helper Function
// Usage: Iterate through a bucket of keys
// Return: True if key found, otherwise false
// Inputs:
//       - LLIterator: iterator for the bucket
//       - keyval: stores current key value
//       - targetkey: key we are trying to find
static bool FindBucketKeys(LLIterator *iterator,
                           HTKeyValue_t **keyval,
                           HTKey_t targetkey) {
  // Uses LinkedList Iterator
  while (LLIterator_IsValid(iterator)) {
    LLIterator_Get(iterator, (void**) keyval);

    // Case (a): Key Found
    if (targetkey == (*keyval)->key) {
      return true;
    }
...
bool HashTable_Insert(HashTable *table,
  ...
  Verify333(chain != NULL);

  HTKeyValue_t *payloadnew = (HTKeyValue_t*) malloc(sizeof(HTKeyValue_t));

  payloadnew->key = newkeyvalue.key;
  payloadnew->value = newkeyvalue.value;

  // Case (a): Inserting new key in empty chain
  if (LinkedList_NumElements(chain) == 0) {
    LinkedList_Append(chain, (void*) payloadnew);
    table->num_elements += 1;
    return false;
  }

  LLIterator *iterator = LLIterator_Allocate(chain);
  HTKeyValue_t *payloadold;

  // Case (b): Inserting new key when key not found in chain
  if (!FindBucketKeys(iterator, &payloadold, newkeyvalue.key)) {
    LinkedList_Append(chain, (void*) payloadnew);
    free(iterator);
    table->num_elements += 1;
    return false;
  ...
  // Case (c): Updating old (key, value) pair
  oldkeyvalue->key = payloadold->key;
  oldkeyvalue->value = payloadold->value;
  (*payloadold).value = newkeyvalue.value;
...
bool HashTable_Find(HashTable *table,
  ...
  LLIterator *iterator = LLIterator_Allocate(chain);
  HTKeyValue_t* temporary;

  // Case (a): Key not found
  if (!FindBucketKeys(iterator, &temporary, key)) {
    free(iterator);
    return false;
  }

  // Case (b): Key Found
  keyvalue->key = temporary->key;
  keyvalue->value = temporary->value;
...
bool HashTable_Remove(HashTable *table,
  ...
  // Case (a): Key not found, nothing to remove
  if (!FindBucketKeys(iterator, &temporary, key)) {
    free(iterator);
    return false;
  }

  // Case (b): Key found so return and remove (key, value) pair
  keyvalue->key = temporary->key;
  keyvalue->value = temporary->value;

  LLIterator_Remove(iterator, LLNoOpFree);  // Removes and deallocates the node
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // Case (a): Iterator not valid
  if (HTIterator_IsValid(iter) == false) {
    return false;
  }

  // Case (b): Iterator cannot be advanced in the current bucket
  if (!LLIterator_Next(iter->bucket_it)) {
    LLIterator_Free(iter->bucket_it);
    iter->bucket_idx += 1;

    // Find next non-empty bucket in the table (if it exists)
    ...
    // No more elements available in the table after searching all buckets
    iter->bucket_it = NULL;
    iter->bucket_idx = INVALID_IDX;
    return false;
  }

  // Case (c): Successful Advancement of Iterator
  return true;
}
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  // Case (b): Retrieve (key, value) pair from current bucket
  LLIterator_Get(iter->bucket_it, (void**) &temporary);
  keyvalue->key = temporary->key;
  keyvalue->value = temporary->value;
