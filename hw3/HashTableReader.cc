...
HashTableReader::HashTableReader(FILE* f, IndexFileOffset_t offset)
  ...
  // Read from Location
  int read_res = fread(&header_, sizeof(BucketListHeader), 1, f);
...
HashTableReader::LookupElementPositions(HTKey_t hash_key) const {
  ...
  // Seek to Location
  int seek_res = fseek(file_, bucket_rec_offset, SEEK_SET);
  Verify333(seek_res == 0);
  ...
  int num_elements = bucket_rec.chain_num_elements;
  if (num_elements == 0) {
    return ret_val;
  }

  for (int i = 0; i < num_elements; i++) {
    ElementPositionRecord elem_pos_record;

    // Calcualte offset for seeking and read
    int offset = bucket_rec.position + sizeof(ElementPositionRecord) * i;

    // Verify valid seek
    seek_res = fseek(file_, offset, SEEK_SET);
    Verify333(seek_res == 0);

    // Verify valid read
    read_res = fread(&elem_pos_record, sizeof(ElementPositionRecord), 1, file_);
    Verify333(read_res == 1);

    // Format conversion and append to back of return list
    elem_pos_record.ToHostFormat();
    ret_val.push_back(elem_pos_record.position);
  }
