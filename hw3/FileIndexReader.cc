...
FileIndexReader::FileIndexReader(const string& file_name,
  ...
  int seek_res = fseek(file_, 0, SEEK_SET);
  ...
  uint32_t header_magic_number = header_.magic_number;
  Verify333(header_magic_number == kMagicNumber);
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      int offset = sizeof(IndexFileHeader) + header_.doctable_bytes
        + header_.index_bytes - left_to_read;

      // Seek to offset found via pointer arithmetic
      seek_res = fseek(file_, offset, SEEK_SET);
      Verify333(seek_res == 0);

      // Read to buffer
      int bytes_per_read = fread(buf, 1, kBufSize, file_);
      Verify333(bytes_per_read != 0);

      for (int i = 0; i < bytes_per_read; i++) {
        crc_obj.FoldByteIntoCRC(buf[i]);
      }

      left_to_read -= bytes_per_read;
    }
