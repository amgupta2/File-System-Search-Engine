...
// Middle ground between low memory but more IO and high memory but low IO
...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  // Follow the same pattern as writing document table
  ...
  // Follow same process of dt_bytes and mi_bytes minus moving the cur_pos
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  int bytes_remaining = doctable_bytes + memidx_bytes;

  while (bytes_remaining > 0) {
    // Calculate proper offset by skipping over index header bytes
    int offset = doctable_bytes + memidx_bytes + sizeof(IndexFileHeader)
      - bytes_remaining;

    // Seek to offset in the file to sart reading
    int seek_res = fseek(f, offset, SEEK_SET);
    if (seek_res != 0) {
      return kFailedWrite;
    }

    // read bytes from seek position to seek position + buffer size
    int bytes_read = fread(buffer, 1, kBufferSize, f);

    Verify333(bytes_read != 0);

    // For every byte in the buffer, calculate checksum incrementally
    for (int i = 0; i < bytes_read; i++) {
      crc.FoldByteIntoCRC(buffer[i]);
    }

    // Update the number of bytes read at each iteration
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    // For each linked list, get the number of elements and write a bucket_rec
    LinkedList* ll_iteration = (ht->buckets)[i];
    int num_elements = LinkedList_NumElements(ll_iteration);

    if (WriteHTBucketRecord(f, record_pos, num_elements, bucket_pos) < 0) {
      return kFailedWrite;
    }
    ...
    if (num_elements != 0) {
      // If there are elements, write the bucket as well
      int bytes_written = WriteHTBucket(f, bucket_pos, ll_iteration, fn);
...
static int WriteHTBucketRecord(FILE* f,
  ...
  // STEP 6.
  // Write the BucketRecord.
  int bytes_written = fwrite(&bucket_record, sizeof(BucketRecord), 1, f);
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int seek_res = fseek(f, record_pos, SEEK_SET);
    ...
    // Since the seek did not fail, we write the element record
    ElementPositionRecord element_record(element_pos);
    element_record.ToDiskFormat();

    int elements_written = fwrite(&element_record,
      sizeof(ElementPositionRecord), 1, f);
    ...
    LLIterator_Get(it, &ll_payload);

    // Write the element itself using the provided function for writing
    // Need to typcast the payload into a type that can be used with our HT
    HTKeyValue_t* casted_payload = reinterpret_cast<HTKeyValue_t*>(ll_payload);
    int write_res = fn(f, element_pos, casted_payload);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  int16_t file_name_bytes = static_cast<int16_t>(strlen(filename));
  ...
  int elem_written = fwrite(kv->value, file_name_bytes, 1, f);
  if (elem_written != 1) {
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  // Create the header in disk format
  DocIDElementHeader element_header(doc_id, num_positions);
  element_header.ToDiskFormat();

  // Write the header
  int elem_written = fwrite(&element_header, sizeof(DocIDElementHeader), 1, f);
  if (elem_written != 1) {
    return kFailedWrite;
  }
  ...
  int total_data_written = sizeof(DocIDElementHeader);
  for (int i = 0; i < num_positions; i++) {
    // STEP 13.
    // Get the next position from the list.
    LLPayload_t ll_payload;
    LLIterator_Get(it, &ll_payload);
    uint64_t payload = reinterpret_cast<uint64_t>(ll_payload);

    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    DocPositionOffset_t truncate = static_cast<DocPositionOffset_t>(payload);
    position.position = truncate;
    position.ToDiskFormat();

    // Seek to correct spot
    int seek_res = fseek(f, offset + total_data_written, SEEK_SET);
    if (seek_res != 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }

    // Write in seek location
    int elem_written = fwrite(&position, sizeof(DocIDElementPosition), 1, f);
    if (elem_written != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }

    total_data_written += sizeof(DocIDElementPosition);
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  // Write at Correct Location
  int elem_written = fwrite(&header, sizeof(WordPostingsHeader), 1, f);
  if (elem_written != 1) {
    return kFailedWrite;
  }
  ...
  // Seek to Correct Location
  seek_res = fseek(f, offset + sizeof(WordPostingsHeader), SEEK_SET);
  if (seek_res != 0) {
    return kFailedWrite;
  }

  // Perform Write
  elem_written = fwrite(wp->word, word_bytes, 1, f);
  if (elem_written != 1) {
    return kFailedWrite;
  }
