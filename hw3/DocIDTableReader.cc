...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    int seek_res = fseek(file_, curr_element, SEEK_SET);
    Verify333(seek_res == 0);

    int read_res = fread(&curr_header, sizeof(DocIDElementHeader), 1, file_);
    Verify333(read_res == 1);
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      std::list<DocPositionOffset_t> offset_list;
      int32_t num_iterations = curr_header.num_positions;

      for (int i = 0; i < num_iterations; i++) {
        DocIDElementPosition elem_pos;

        read_res = fread(&elem_pos, sizeof(DocIDElementPosition), 1, file_);
        Verify333(read_res == 1);

        elem_pos.ToHostFormat();
        offset_list.push_back(elem_pos.position);
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file.
    int offset = offset_ + sizeof(BucketListHeader) + sizeof(BucketRecord) * i;
    int seek_res = fseek(file_, offset, SEEK_SET);
    Verify333(seek_res == 0);
    ...
    int read_res = fread(&bucket_rec, sizeof(BucketRecord), 1, file_);
    Verify333(read_res == 1);
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      read_res = fread(&element_pos, sizeof(ElementPositionRecord), 1, file_);
      Verify333(read_res == 1);

      element_pos.ToHostFormat();

      seek_res = fseek(file_, element_pos.position, SEEK_SET);
      Verify333(seek_res == 0);
      ...
      read_res = fread(&element, sizeof(DocIDElementHeader), 1, file_);
      Verify333(read_res);
