...
bool FileReader::ReadFile(string *const contents) {
  ...
  // If the path is not in the base directory, it is invalid
  if (!IsPathSafe(basedir_, full_file)) {
    return false;
  }

  int size = 0;
  // Read file into memory using unique_ptr for automatic memory management
  std::unique_ptr<char, decltype(&free)>
    file_content(::ReadFileToString(full_file.c_str(), &size), &free);

  // If the file is empty, the read failed
  if (!file_content) {
    return false;
  }

  // Set return param to file contents
  *contents = std::string(file_content.get(), size);
  return true;
}
