  ...
  while (dirent != NULL) {
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      if (S_ISDIR(st.st_mode)) {  // directory
        ...
        entries[i].is_dir = 0;
      } else {  // neither so skip
        dirent = readdir(d);
        continue;
      }
    }

    // Increment Tracking Variables and Move To Next Entry
    i += 1;
    dirent = readdir(d);
  }  // end iteration over directory contents ("first pass").

  // Finished iterating over the directory --> Cleanup
  closedir(d);
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  // of the file.
  char* content = ReadFileToString(file_path, &file_len);
  tab = ParseIntoWordPositionsTable(content);
