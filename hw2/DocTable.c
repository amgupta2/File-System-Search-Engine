...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  if (DocTable_GetDocID(table, doc_name) != INVALID_DOCID) {
    return DocTable_GetDocID(table, doc_name);
  }

  // Allocate space for doc_name copy
  doc_copy = (char *) malloc(strlen(doc_name) + 1);
  Verify333(doc_copy != NULL);

  // Allocate space for new doc_id
  ...
  *doc_id = table->max_id;
  ...
  // the provided code.
  res = FNVHash64((unsigned char*) doc_copy, strlen(doc_copy));
  kv.key = res;
  kv.value = doc_id;
...
DocID_t DocTable_GetDocID(DocTable* table, char* doc_name) {
  ...
  if (HashTable_Find(table->name_to_id, key, &kv)) {  // doc found
    return *(DocID_t *) (kv.value);
  }

  return INVALID_DOCID;  // doc not found so invalid id
}
...
char* DocTable_GetDocName(DocTable* table, DocID_t doc_id) {
  ...
  if (HashTable_Find(table->id_to_name, doc_id, &kv)) {  // doc found
    return kv.value;
  }
