...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    // (1): Allocate memory for WPs and assign word
    wp = (WordPostings*) malloc(sizeof(WordPostings));
    wp->word = word;

    // (2): Allocate new hashtable for WP's docID->posting mapping

    // start with small # of buckets --> follow MemIndex_Allocate
    wp->postings = HashTable_Allocate(16);
    mi_kv.key = key;
    mi_kv.value = wp;

    // (3): insert WP into inverted index
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  if (!HashTable_Find(index, key, &kv)) {  // key not found --> cleanup
  ...
  while (HTIterator_Get(it, &kv)) {
    // Allocate SearchResult
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      // If a result is found, add the new occurances to the rank
