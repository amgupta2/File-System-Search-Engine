...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  while (left_to_read > 0) {
    ...
    if (result == -1) {  // Error Case - Determine recoverable vs unrecoverable
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  while (1) {
    if (*cur_ptr == '\0') {  // reached end of content
      break;
    } else if (isalpha(*cur_ptr) != 0) {  // character is an alphabetic
      *cur_ptr = tolower(*cur_ptr);
    } else {  // character is not alphabetic
      *cur_ptr = '\0';

      if (cur_ptr != content && isalpha(*(cur_ptr - 1)) != 0) {
        // Add to hashmap if the previous character was alphabetic
        // Since that is the end of that word
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // Create space for WordPositions
    wp = (WordPositions *) malloc(sizeof(WordPositions));
    Verify333(wp != NULL);

    // Create Space for String in WordPositions
    new_string = (char *) malloc(strlen(word) + 1);
    Verify333(new_string != NULL);

    // Create space for LinkedList in WordPositions
    wp->positions = LinkedList_Allocate();
    Verify333(wp->positions != NULL);

    // Copy the arguemtn word passed in to the struct
    strncpy(new_string, word, strlen(word) + 1);
    wp->word = new_string;

    // Append the position of string to the positions list
    LinkedList_Append(wp->positions, (LLPayload_t) (int64_t) pos);
